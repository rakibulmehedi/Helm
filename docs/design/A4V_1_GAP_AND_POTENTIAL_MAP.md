...
Pocketa has already solved the hardest problems:

1. **Token system is perfect** -- 13 colors, 18 typography styles, 8pt spacing grid, motion tokens. All correct.
2. **Core widgets are excellent** -- 13 Pocketa-prefixed widgets, all fully token-compliant.
3. **Dashboard cockpit is near-doctrine** -- PocketaRealityStack, S2sHeroBlock, PocketaCalculationTrace, PocketaLedgerRail, PocketaTrustStrip.
...
**The hidden product power is the S2S cascade.** It is already computed correctly. It is already displayed correctly on the dashboard. But every other screen that touches financial data (income list, add income, pipeline, settings) renders numbers without monospace fonts, without PocketaAmount, without ledger rails, without trust strips. The product's intelligence is present but visually invisible outside the home screen.
...
**Potential:** Rewriting `button_multiple_types.dart` to use `PocketaColors.interactive` (#255E5B) will change the visual identity of every action button in the app from "Flutter template blue" to "calm financial deep teal." This one change affects: onboarding (6 pages), welcome, income screens, transaction screen, settings, and confirm-received sheet.
...
**Potential:** Phosphor outline icons (1.5pt stroke) would give Pocketa a unique visual signature. Material Icons are recognizable as "default Flutter." Phosphor icons are less common, cleaner, and match the ledger-calm aesthetic. The 1.5pt stroke weight aligns with the hairline border philosophy.
...
**Potential:** When every text element uses PocketaTypography, the app will have a visible type rhythm. Headers will be consistently headingMd/headingLg. Body text will be consistently bodyMd/bodyLg. Labels will be consistently labelSm/labelMd. Financial numbers will be consistently monoFinancial*. This rhythm is what separates a designed app from a coded app.
...
**Gap:** Income list is the worst frequently-seen screen (score: 30/100). 14+ hardcoded fonts, italic notes, generic filter chips, card radius 16 instead of 12, no PocketaAmount for money values.
**Potential:** This screen shows the user's actual money pipeline entries. If rebuilt with PocketaLedgerCard for entries, PocketaAmount for values, PocketaSourceCard for pipeline items, and proper token usage throughout, it would feel like a real financial ledger. The data is already there -- the rendering doesn't do it justice.
...
**Potential:** Removing categories and replacing with simple "Cash out" recording (amount + note + date) would:
  ...
  - Align with Final Product Doctrine ("Pocketa is not a backward-looking expense tracker")
...
**Potential:** A clean splash with:
  ...
  - "Pocketa" in Inter w600, inkPrimary color, centered
  - 300ms fade-in using PocketaMotion.slow
...
### 8. PocketaAmount Adoption for All Money Values

**Gap:** PocketaAmount widget exists and correctly renders monospace BDT-formatted numbers. But it's only used in the dashboard (S2sHeroBlock, CommittedSection, ReserveSection, NotCountedSection, CalcTrace). Income list, add income, pipeline summary, and STS settings all render money values with proportional fonts and inconsistent formatting.

**Potential:** If every money value in the app used PocketaAmount, financial figures would be:
...
**Why it's suppressed:** It's hidden behind a tap with only a tiny hint ("Tap the number to see the math"). The visual treatment is correct (PocketaAuditCard, stagger animation, PocketaMotion). But discovery depends on the user tapping the right thing.

**How to surface it:** The Trust Strip ("Updated 11:42 PM . Received only") already links to the trace via `onTapAudit`. Consider making the entire S2S hero zone tappable (it is via S2sHeroBlock's PocketaHeroZone). The one-time hint helps, but a persistent "See math" link in the Trust Strip would improve discovery.
...
**How to surface it:** `PocketaLedgerRail` widget is ready. Just needs adoption in more contexts.
...
**What it is:** PocketaAmount renders money as `tk 36,000.00` -- taka prefix, lakh formatting, monospace JetBrains Mono.
...
| PocketaAmount adoption | Dashboard only | All money values | MAJOR | P1 |
...
1. Rewrite AppButton -> PocketaColors.interactive
...
1. Replace 30+ ResponsiveUtilities.font() with PocketaTypography tokens
...
1. Replace 80+ hardcoded BorderRadius with PocketaSpacing tokens
2. Remove 4 BoxShadow instances
3. Standardize EdgeInsets to PocketaSpacing
...
1. Splash redesign
2. Income list rebuild with PocketaLedgerCard + PocketaAmount
